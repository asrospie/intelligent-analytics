...
def build_model(n_units, n_layers, learning_rate, dropout):
...
CALLBACKS = [keras.callbacks.EarlyStopping(monitor='mean_squared_error', patience=5)]
...
tuner.search(x_scaled_full, y_scaled_full, epochs=50, callbacks=CALLBACKS)
...
for idx, m in enumerate(best_models):
    ...
    for train, valid in loo.split(x_scaled_full):
        ...
        m.fit(x_train, y_train, epochs=50, verbose=0, callbacks=CALLBACKS)
